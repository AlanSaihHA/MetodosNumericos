...
REAL*4, ALLOCATABLE :: x(:),xc(:),y(:),yc(:),P(:,:),Pp(:,:),d_h(:,:),d_v(:,:),uc(:,:),vc(:,:),psi(:,:)
...
ALLOCATE(uc(0:nx+1,0:ny+1),vc(0:nx+1,0:ny+1),d_h(0:nx+1,0:ny+1),d_v(0:nx+1,0:ny+1),psi(0:nx+1,0:ny+1))
...
OPEN(1,FILE='./vel/anim.gnp',STATUS='REPLACE')
...
aP=0.0; aE=0.0; aW=0.0; aN=0.0; aS=0.0; Sp=0.0; Pp=0.0
  ...
  IF (MOD(it,100) .EQ. 0) THEN
    ...
    CALL WriteVectorField('./vel/vel',it,uc,vc,xc,yc,nx,ny)
    CALL WritePsi('./stream/psi',it,psi,xc,yc,nx,ny)
    CALL WriteVorticity('./vorticity/omega',it,omega,xc,yc,nx,ny)
