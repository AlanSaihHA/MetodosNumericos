Program Laplace 
  ...
  allocate(T(0:nx+1,0:ny+1),Ta(0:nx+1,0:ny+1),aP(nx,ny),aE(nx,ny),aW(nx,ny),&
  	aS(nx,ny),aN(nx,ny),SP(nx,ny),xc(0:nx+1),x(0:nx),yc(0:ny+1),y(0:ny))
  ...
  do i=1,ny
     do j=1,nx
        ...
        aN(i,j)=k*Ss/dy
        aS(i,j)=k*Sn/dy       
        SP(i,j)=0.0 !En este caso dv=dx*dy
  ...
  call  WriteScalarField2D('Temp2D-num',0,T,xc,yc,nx,ny)
  

do j=0, ny+1
	do i=0, nx+1
		Ta(i,j)=T0*sin(pi*xc(i)/(xl-x0))*(sinh(pi*yc(j)/(yl-y0))/sinh(pi))
	enddo
enddo

  call  WriteScalarField2D('Temp2D-ana',0,Ta,xc,yc,nx,ny) 
  !!Solución analitica
!!$  Ta(0)=Th
!!$  Ta(nx+1)=Tc
!!$
