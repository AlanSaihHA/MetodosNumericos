...
!!!LID DRIVEN CAVITY
  ...
  real, allocatable::thc(:),th(:),rc(:),r(:),u1(:,:),v1(:,:),u(:,:),v(:,:),uc(:,:),vc(:,:),T(:,:)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	do i=1,nth
		do j=1,nr
			...
			aN(i,j) = Sn/dr - 0.5*un*Sn
			aS(i,j) = Ss/dr + 0.5*us*Ss
	...
	!ESTE
	aP(nth,:) = aP(nth,:) + aE(nth,:)
	Sp(nth,1:nr) = Sp(nth,1:nr) + 2.0*aE(nth,1:nr)*T(nth+1,1:nr)
	aE(nth,:) = 0.0
	!OESTE
	aP(1,:) = aP(1,:) + aW(1,:)
	...
	!NORTE
	aP(:,nr) = aP(:,nr) + aN(:,nr)
	Sp(1:nth,nr) = Sp(1:nth,nr) + 2.0*aN(1:nth,nr)*T(1:nth,nr+1)
	aN(:,nr) = 0.0
	!SUR
	aP(1:nth,1) = aP(1:nth,1) + aS(1:nth,1)
	Sp(1:nth,1) = Sp(1:nth,1) + 2.0*aS(1:nth,1)*T(1:nth,0)
	aS(:,1) = 0.0
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        do j=bj,ej
	      ...
	      !Áreas
              ...
              aE(i,j)=Pr*Se/(rp*dth)-0.5*(ue*Se)
              aW(i,j)=Pr*Sw/(rp*dth) +0.5*(uw*Sw)              
              aN(i,j)=Pr*Sn/dr -0.5*(un*Sn)              
...
enddo
  ...
  !para el archivo de animación de velocidad
    WRITE(itchar,'(i6)')it    
 ...
 !   a=a+1.0
   ...
   call WriteScalarField2D('T',it,T,thc,rc,nth,nr)
