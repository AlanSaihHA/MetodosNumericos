...
DO it=1,itmax
  ...
  DO i=1,ei
    DO j=1,ej
      ...
      aS(i,j)=gamma*Ss/dy + max(vs*Ss,0.0)
...
aP=0.0; aE=0.0; aW=0.0; aN=0.0; aS=0.0; Sp=0.0; Pp=0.0
  ...
  IF ((it*dt)==1.4 .or. (it*dt)==2.4-0.005 .or. (it*dt)==3.4-0.005 &
  .or. (it*dt)==4.4 .or. (it*dt)==5.4 .or. (it*dt)==6.4-0.005   &
  .or. (it*dt)==7.4-0.005 .or. (it*dt)==8.4 .or. (it*dt)==9.4 &
  .or. (it*dt)==12.6-0.01 .or. (it*dt)==15. .or. (it*dt)==20.) THEN 
  ...
  !para el archivo de animación
   ...
   !archivo propio para el tiempo indicado
    OPEN(5,FILE=""//itchar(1:LEN_TRIM(itchar))//".gp",STATUS='REPLACE')
    WRITE(5,*)'set size square; set xrange[0:1]; set yrange[0:1]; unset key'
