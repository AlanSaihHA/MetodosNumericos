  ...
  do i=1,ny
     do j=1,nx
		...
		if (uw>0) then
			 ...
			 aW(i,j) = gamma*Sw/dx + (3/8)*uw
		end if
		
		if (ue>0) then
			 aE(i,j) = gamma*Se/dx - (3/8)*ue
		else 
			 aW(i,j) = gamma*Sw/dx - (6/8)*ue -(1/8)*uw
		end if
		
		if (us>0) then
			 aS(i,j) = gamma*Ss/dx + (1/8)*un + (6/8)*us
		else 
			 aS(i,j) = gamma*Ss/dx + (3/8)*us
		end if
		
		if (un>0) then
			 aN(i,j) = gamma*Sn/dx - (3/8)*un
		else 
			 aN(i,j) = gamma*Sn/dx - (6/8)*un -(1/8)*us
		end if
