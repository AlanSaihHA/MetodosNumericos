Program Laplace
  ...
  aP=0.;aE=0.;aW=0.; aS=0.; aN=0.; aWW=0.; aSS=0.; aEE=0.; aNN=0.; SP=0.; T=0.;u=0.;v=0.
  ...
  do it=1,itmax
     do i=1,nx
        ...
        write(*,*) uw, ue, us, un
	if (uw>0) then
		 aW(i,j) = k*Sw/dx + (1/8)*ue + (6/8)*uw
	else 
		 aW(i,j) = k*Sw/dx + (3/8)*uw
	end if
		
	if (ue>0) then
		 aE(i,j) = k*Se/dx - (3/8)*ue
	else 
		 aW(i,j) = k*Sw/dx - (6/8)*ue -(1/8)*uw
	end if
		
	if (us>0) then
		 aS(i,j) = k*Ss/dx + (1/8)*un + (6/8)*us
	else 
		 aS(i,j) = k*Ss/dx + (3/8)*us
	end if
		
	if (un>0) then
		 aN(i,j) = k*Sn/dx - (3/8)*un
	else 
		 aN(i,j) = k*Sn/dx - (6/8)*un -(1/8)*us
	end if
	...
	if (uw>0 .and. ue>0 .and. us>0 .and. un>0) then
		 aWW(i,j) = -(1/8)*uw
		 aSS(i,j) = - (1/8)*us
	end if
	...
	if (uw<0 .and. ue<0 .and. us<0 .and. un<0) then
        ...
        aP(i,j)=aE(i,j) + aW(i,j) +  aN(i,j) +  aS(i,j) + aWW(i,j) + aEE(i,j) + aSS(i,j) + aNN(i,j) + dv/dt
     enddo    
  enddo
