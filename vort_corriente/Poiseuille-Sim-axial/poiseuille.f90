...
!  ue=0.0;uw=0.0;un=0.0;us=0.0;
  ...
  Se=dy; Sw=dy; Sn=dx; Ss=dx; dv=dx*dy  
  !u = 1.0
  u(:,ny+1)=0.0; u(:,0)=u(:,1)
  u1=u;v1=v
!  do it=1,itmax !ciclo temporal
  ...
  do while ((it.LE.itmax).AND.(flag.EQ.0))
     ...
     !cara norte
     do i=0,nx+1
         vor(i,ny+1)=-2.0*(psi(i,ny)-psi(i,ny+1))/(yc(ny+1)*dy*dy) !-4.0*u(i,ny+1)/dy  
     ...
     !cara oeste
     do j=0,ny+1
         vor(0,j)= -2.0*(psi(1,j)-psi(0,j))/(yc(j)*dx*dx) + 1.0/yc(j)  !Ultimo término 1.0/yc(j)    !-8.0*(-psi(0,j)+psi(1,j))/(dx*dx)
     ...
     do i=1,nx
        do j=1,ny
           ...
           SP(i,j)= SP(i,j) + (2.*w(i,j)/rp)*((w(i+1,j)-w(i-1,j))/(2.0*dx))*dv &
     ...
     do i=1,nx
        do j=1,ny
           ...
           aN(i,j)=eps*(Sn/dy)-0.50*(un*Sn)
           aS(i,j)=eps*(Ss/dy)+0.50*(us*Ss)      
           SP(i,j)=w(i,j)*dv/dt !En este caso dv=dx*dy
     ...
     !corrección a las fronteras
     !Cara Este 
     !SP(nx,:)=SP(nx,:) - aE(nx,:)*w(nx+1,1:ny)      !Condicion Neumann
     aP(nx,:)=aP(nx,:) - aE(nx,:) 
     aE(nx,:)=0.0 

     !Cara Oeste
     SP(1,:)=sp(1,:)+2.0*aW(1,:)*w(0,1:ny)   !Condicion Dirichlet
     aP(1,:)=aP(1,:)+aW(1,:)
     aW(1,:)=0.0

     !Cara Norte  
     SP(:,ny)=SP(:,ny)+2.0*aN(:,ny)*w(1:nx,ny+1)    !Condicion Dirichlet
     aP(:,ny)=aP(:,ny)+aN(:,ny)
     aN(:,ny)=0.0
     ...
     call Gauss_TDMA2D(w,nx,ny,aP,aE,aW,aN,aS,sP,nx,ny,max_iter,tolerance,residual) 
     w(nx+1,:) = w(nx,:)    !Correcion por frontera Neumann
     !!!!!!!!!!!!!!!!Resolviendo Corriente!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     call Stream(psi,nx,ny,x0,xl,y0,yl,yc,y, vor,tolerance,max_iter,residual)
     !esto es valido solo para los nodos internos 2,nx-1 y 2,ny-1.
     !falta correjir para las fronteras 
     do i=2,nx-1
        do j=2,ny-1
           rp = yc(j)
           u(i,j) =(1./rp)*(psi(i,j+1)-psi(i,j-1))/(dy*2.0)           
...
!!****************************************************************************************************************
subroutine  Stream(psi,nx,ny,x0,xl,y0,yl,yc,y,vor,tolerance,max_iter,residual)
  implicit none
  ...
  psi(1:nx,0) = yc(0)*yc(0)/2.
  !cara norte
  psi(:,ny+1)= yc(ny+1)*yc(ny+1)/2.0
  !definiendo las áreas
  Se=dy; Sw=dy; Sn=dx; Ss=dx
  !**********Comienza a resolverse la ecuación de Poisson para vorticidad***********
  do i=1,nx
     do j=1,ny
        rp = yc(j); rn=y(j); rs=y(j-1)
        aE(i,j)=Se/(rp*dx)
        aW(i,j)=Sw/(rp*dx)
        aN(i,j)=Sn/(rn*dy)
        aS(i,j)=Ss/(rs*dy)
        SP(i,j)=vor(i,j)*dv - (psi(i,j)/(rp*rp*rp))*dv              !En este caso dv=dx*dy
