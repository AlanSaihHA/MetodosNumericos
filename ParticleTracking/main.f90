...
program LD2DNS
  ...
  real, dimension(:,:), allocatable :: B0z
	...
	nx1=1 ; nx2=nx ; ny1=1 ; ny2=ny !  Boundary cell values
	Re=3.0
	Ga=0.7
	gma=0.1
...
! Calling magnetic field subroutine
  ...
  call MagneticField(nx,ny,nx1,nx2,ny1,ny2,dx,dy,lx,ly,B0z)
  B0z=B0z/maxval(B0z)     ! Magnetic field normalization
...
! ------------------------------------------------------------
! B0z field subroutine ---------------------------------------
subroutine MagneticField(nx,ny,nx1,nx2,ny1,ny2,dx,dy,lx,ly,B0z)
implicit none
