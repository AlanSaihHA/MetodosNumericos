...
!----------------------------------------------------------------------------
	...
	nxp2=514  !numero de volumenes en x
        nyp2=258  !numero de volumenes  en y
	maxel=1000
	maxpt=1000
...
!======================================================================================	  
	  ...
	  r2=1.0d0     !densidad del fluido
	  Ifi1=0.0d0    !campo indicador en el sólido
	  Ifi2=1.0d0     !campo indicador en el fluido
	  gx=0.0d0
	  gy=0.0d0
	  dt=0.0005d0    !Calculado por criterio de Kuran
	  Re=100.0d0
	  gama=1.0d0/Re
	  xcc=4.0d0     !Posicion del objeto solido en x
	  ycc=4.0d0     !posicion del objeto solido en y
	  

!=======================================================================================
!				Pressure Solution data
!======================================================================================	  
	  itmax=1000             !numero maximo de iteraciones
	  xerr=0.000001d0         
	  beta=1.2              !factor sobrerelajacion sor
!=======================================================================================
!				Boundary conditions
!========================================================================================
	  ibdry=0    !0 ciclicas
	...
	ucco=0.0d0    !velocidad inicial del cuerpo en x
	vcco=0.0d0    !velocidad inicial del cuerpo en y

	npvel=1
	nppr=1
	nback=6000000	  
          write(*,*)' TYPE name of output files' !Escritua directorio donde se guardan datos
	     read (5,'(a10)')fname    !nombre carpeta donde se guardan
         lfnm=len_trim(fname)+1   !longitud de cadena de nombre
         
       command='mkdir '
       command(7:7+lfnm)=fname   !Se escribe en espacio 7 el nombre de la carpeta
...
! SET PARAMETERS FOR FRONT RESTRUCTURING
      ...
      call finit(nfronts,fp,ip,pt,icp,ine,ptcon,elcon,bptcon,belcon,elprop,maxpt,maxel,am,prop1,prop2,xcc,ycc)  !crea el circulo/cilindro
      call fcurv(cv,t1,t2,cnt,pt,icp,ine,ptcon,elcon,maxel,maxpt,norm)  !No se ocupa aquí pero sirve para calcular la curvatura y las normales

	k=ffe 
	do ll=1,ne	   !Se escriben vectores normales en un archivo de datos
...
!Initial conditions	
	u=1.0d0
	v=0.0d0
	do i=1,nxp2  !Poniendo velocidad cero dentro del solido
	do j=1,nyp2
...
!================================================================
	...
	time=float(nstep)*dt
	call Forceh(pt,icp,elcon,maxel,maxpt,norm,u,v,p,Fhx,Fhy,cnt,gama,nxp2,nyp2,r,xcc,ycc)  !Calculo de fuerza hidrodinamica
...
! generate gradient vector
      ...
      k=ffe
      do ll=1,ne 
       p0x=pt(icp(k,1),1)   !un extremo en x a partir de un punto, icp(i+netot,1)=i +nptot
       p0y=pt(icp(k,1),2)   !un extremo en y a partir de un punto, icp(i+netot,1)=i +nptot
       p1x= p0x+dmod((pt(icp(k,2),1)-p0x+xx),xl)-0.5d0*xl  !icp(k,2) es el extremo siguiente, icp(i+netot,2)=i+1+nptot, 
       p1y= p0y+dmod((pt(icp(k,2),2)-p0y+yy),yl)-0.5d0*yl

       xc=0.5*(p1x+p0x)   !punto promedio de los puntos, centro del elemento en x
       yc=0.5*(p1y+p0y)   !punto promedio de los puntos, centro del elemento en y
       x1=p1x-p0x  !Es delta L en Ec. 2.48
...
! generate a grid value   
       ...
       do i1=1,4
       do j1=1,4
        ii=ir-2+i1   !es el i de las celdas cerca de la interface
        jj=jr-2+j1

         drx = 1. + cos((xt*hxi - float(ii-1))*pd2)   !parte ec-(2.46), (1+cos(pi*r/2h))
         dry = 1. + cos((yt*hyi - float(jj-1))*pd2)

        tmp1(ii+1,jj+1)=tmp1(ii+1,jj+1)+rx*drx*dry*cnstx !toda ec.(2.48), es decir el gradiente
        tmp2(ii+1,jj+1)=tmp2(ii+1,jj+1)+ry*drx*dry*cnsty
       enddo
       enddo

      k=elcon(k)  !Elemento conectado elcon(i)=i+1
      enddo   
      ...
      call gbd1(tmp1,tmp2,nxp2,nyp2)  ! Se utiliza para especificar las fronteras
       ...
       do j=2,nyp1
        rtmp(i,j)= 0.5d0*(hxi*(tmp1(i+1+1,j+1)-tmp1(i-1+1,j+1))+		&
             hyi*(tmp2(i+1,j+1+1)-tmp2(i+1,j-1+1)))  !Se obtiene la divergencia. lado derecho de ec. 2.49
       enddo
       enddo
...
!                         { itp=1 for a cylinder}
        ...
        do i=1,nps 
         ...
         pt(i+nptot,1)=xcc + rad*cos(th)  !localizacion de puntos en la interface
         pt(i+nptot,2)=ycc + rad*sin(th)  !nptot es para poner varios objetos inmersos
        enddo 
 
       do i=1,nps                       ! arreglo de puntos con puntos de los alrededores
           icp(i+netot,1)=i    +nptot   !icp es con los puntos, intersecciones de lineas
           icp(i+netot,2)=i+1  +nptot
           ine(i+netot,1)=i-1  +netot   !ine es con los elementos, segmentos de linea
           ine(i+netot,2)=i+1  +netot 
             elprop(i+netot,1)=prop1   !prop1 = r1-r2, r1 = dens del solido, dens del fluido
             elprop(i+netot,2)=prop2   !prop2 = 0.0
       enddo  
           icp(nps+netot,2)=1  +nptot   !ultimo punto con primer punto
           ine(1+netot,1)=nps  +netot   !primer elemento con ultimo elemento
           ine(nps+netot,2)=1  +netot   !ultimo elemento con primer elemento
     ...
     enddo
      ...
      ffe=1    !para elementos
      lfe=ne
      fee=ne+1

! set connectivity

      do i = 1,maxpt-1   !Se comienza conexion entre puntos
       ptcon(i)=i+1   !punto con punto siguiente
       bptcon(i+1)=i  !punto con punto anterior/back
     enddo
       ptcon(lfp)=1    !punto ultimo con primer punto
       bptcon(1)=lfp   !punto primero con ultimo

      do i = 1,maxel-1      !se comienza conexion entre elementos
       elcon(i)=i+1        !elemento con elemento siguiente
       belcon(i+1)=i      !elemento con elemento anterior/back
      enddo
       elcon(lfe)=1     !ultimo elemento con primer elemento
       belcon(1)=lfe    !primer elemento con ultimo elemento
